...
import pyaudio
import websocket
from ibm_watson.assistant_v2 import RuntimeIntent, RuntimeEntity, DialogNodeAction, MessageOutputDebug
...
authenticator = IAMAuthenticator('qq5TlEg-CtJcstv_zhtoRCI4_-vKN_ijgKZIomRaN--o')
assistant = AssistantV2(
    version='2020-09-24',
    authenticator = authenticator
)

assistant.set_service_url('https://api.eu-gb.assistant.watson.cloud.ibm.com/instances/95ccabbe-29e3-4318-8255'
                          '-0b5387ff2f0e')


# for TTS service

urll = 'https://api.us-south.text-to-speech.watson.cloud.ibm.com/instances/88dcef8e-d625-454c-ab6f-735ed477f414'
apikeyy = 'k37p8ndFVIENIGEbKHXLKOYC_wJlgOb1konDKfmB_cIV'

authenticator = IAMAuthenticator(apikeyy)
tts = TextToSpeechV1(authenticator=authenticator)
tts.set_service_url(urll)
...
def on_message(self, msg):
    ...
    if "results" in data:
        if data["results"][0]["final"]:
            FINALS.append(data)
            LAST = None
            with open('myaudio.txt', 'r') as f:
                text = f.readlines()
                text = [line.replace('\n', '') for line in text]
                text = ''.join(str(line) for line in text)

            with open('./myaudio.mp3', 'wb') as audio_file:
                res = tts.synthesize(text, accept='audio/mp3', voice='en-US_EmilyV3Voice').get_result()
            ...
            print(p)
            with open('myaudio.txt', 'w')as out:
                out.writelines(p)
